<div>
    ...
    <div class="mb-4 p-3 rounded bg-gray-100 dark:bg-gray-800 flex flex-wrap gap-6 items-center">
        <span class="font-semibold">Rekap Bulan {{ $bulanTahun }}:</span>
        <span>Hadir: <span class="font-bold text-green-600">{{ $rekap['hadir'] }}</span></span>
        <span>Remote: <span class="font-bold text-blue-600">{{ $rekap['remote'] }}</span></span>
        <span>Leave: <span class="font-bold text-yellow-600">{{ $rekap['leave'] }}</span></span>
        <span>Libur: <span class="font-bold text-red-600">{{ $rekap['libur'] }}</span></span>
        <span>Alfa: <span class="font-bold text-red-600">{{ $rekap['alfa'] }}</span></span>
        <span>Total: <span class="font-bold text-blue-600">{{ $rekap['total'] }}</span></span>
    </div>
    @endif
    <style>
    ...
    <div class="mb-4 flex flex-wrap items-end gap-4">
        ...
        <div class="flex gap-2">
            <form action="{{ route('admin.export-attendance-data') }}" method="GET" target="_blank">
            ...
            <button id="export-calendar-image" class="{{ $btnClassBlue }}">
                Export PDF (Gambar Kalender)
            </button>
            <form id="calendar-image-form" action="{{ route('admin.export-attendance-calendar-image') }}" method="POST" target="_blank" style="display:none;">
                @csrf
    ...
    <div id="calendar-export-area">
        ...
        <div class="mb-4 p-4 rounded rekap-export flex flex-wrap gap-6 items-center" style="font-size:1.1em; margin-top:32px;">
            <span class="font-semibold">Rekap Bulan {{ $bulanTahun }}:</span>
            <span>Hadir: <span style="color:#16a34a;font-weight:bold">{{ $rekap['hadir'] }}</span></span>
            <span>Remote: <span style="color:#2563eb;font-weight:bold">{{ $rekap['remote'] }}</span></span>
            <span>Leave: <span style="color:#facc15;font-weight:bold">{{ $rekap['leave'] }}</span></span>
            <span>Libur: <span style="color:#ef4444;font-weight:bold">{{ $rekap['libur'] }}</span></span>
            <span>Alfa: <span style="color:#000;font-weight:bold">{{ $rekap['alfa'] }}</span></span>
            <span>Total: <span style="color:#111827;font-weight:bold">{{ $rekap['total'] }}</span></span>
        </div>
        @endif
        ...
        <x-filament-panels::page>
             {{-- The calendar widget is rendered here by Filament --}}
        </x-filament-panels::page>
    </div>
    <style>
    .rekap-export {
