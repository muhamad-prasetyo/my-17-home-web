<div x-data="{ open: false }" class="relative">
    ...
    <button type="button" class="relative focus:outline-none" @click="open = !open">
        <x-heroicon-o-bell class="w-7 h-7 text-gray-400" />
        @if($unreadCount > 0)
    ...
    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-lg z-50 border border-gray-200 dark:border-gray-700" style="width: 28rem; min-width: 20rem; max-width: 90vw; display: none;">
        ...
        <ul class="max-h-96 overflow-y-auto divide-y divide-gray-200 dark:divide-gray-700">
            @forelse($notifications as $notification)
                ...
                @endphp
                <li>
                    <a href="{{ $url }}" class="flex items-start px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                        <div class="flex-shrink-0">
                            <img src="{{ $data['avatar'] ?? asset('images/default_avatar.png') }}" class="w-10 h-10 rounded-full object-cover" />
                        </div>
                        <div class="ml-5 flex-1 min-w-0" style="margin-left: 1.5rem;">
                            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $data['title'] ?? 'Notifikasi' }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $data['body'] ?? '' }}</p>
        ...
        <div class="p-2 border-t border-gray-200 dark:border-gray-700 text-center">
            <a href="{{ url('/admin/notifications') }}" class="text-xs text-blue-600 hover:underline">Lihat semua notifikasi</a>
        </div>
    </div>
</div> 